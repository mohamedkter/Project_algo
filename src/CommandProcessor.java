...
public class CommandProcessor {
    ...
    public void processor(String line) {
    	...
    	String[] parts = line.split(" ");
    	 KVPair<String, Object> pair;
    	 for (int i = 0; i < parts.length; i++) {
             
             switch(parts[i])
             {
             case "insert":
            	 pair=new KVPair<>(parts[i+1],new Rectangle(parts[i+2].trim(), parts[i+3].trim(),parts[i+4].trim(), parts[i+4].trim()) );
            	 data.insert(pair);
            	 break;
             case "search":
            	 
            	 data.search(parts[i+1]);
            	break;
             case "dump":
            	 data.dump();
            	 break;
             case "remove":
            	 //if(parts[i+2]=="remove"||parts[i+2]=="search"||parts[i+2]=="dump"||parts[i+2]=="insert"||parts[i+2]=="regionsearch"||parts[i+2]=="")
            	 
            		 data.remove(parts[i+1]);
            	 
//            	 else {
//            		 data.removebyvalue(new Rectangle(parts[i+1].trim(), parts[i+2].trim(),parts[i+3].trim(), parts[i+4].trim()));
//            	 }
